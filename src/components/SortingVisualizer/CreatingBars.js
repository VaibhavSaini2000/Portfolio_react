...
import './CreatingBars.css';
function CreatingBars({index,height,color,changeArray}) {
  ...
  // const barStyle = {
	...
	const inputStyle = {
		...
		width: height,
		left: -Math.floor(height / 2) + 13,
		border: 'none',
		background: 'none',
	};

	const bottom = {
		transform: `translateY(${200 - height}px) rotateX(-90deg)`,
	...
	const front_back_right_left = {
		height: `${height}px`,
		transform: `translateY(${200 - height}px)`,
		backgroundColor: `${colors[color][0]}`,
		boxShadow: `5px 5px 50px 5px ${colors[color][1]}`,
		trasistion: '0.3s',
	};
